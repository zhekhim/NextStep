...
void main() {
  test('unequal question counts store rounded percentage scores', () {
    ...
    expect(fields['realistic'], 51);
    expect(fields['investigative'], 62);
    ...
    for (final score in result.rankedScores) {
      expect(profile.percentages[score.dimension], score.percentage.round());
    }
  ...
  test('stored RIASEC code is preserved when rebuilding a result', () {
    final profile = AssessmentProfile(
      ...
      riasecCode: 'RIA',
      percentages: const {'R': 60, 'I': 55, 'A': 50, 'S': 95, 'E': 90, 'C': 85},
    );

    final result = profile.toResult();

    expect(result.code, 'RIA');
    expect(result.rankedScores.map((score) => score.dimension), [
      'R',
      'I',
      'A',
      'S',
